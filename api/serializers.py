...
class EmpleadoCreateSerializer(serializers.ModelSerializer):
    """Serializer para crear/actualizar empleados"""
    password = serializers.CharField(write_only=True, required=False)
    
    class Meta:
        model = Empleado
        fields = ['numero_empleado', 'nombre_completo', 'email', 'id_departamento', 'password', 'activo']
    ...
    def create(self, validated_data):
        ...
        if password:
            ...
            # Si no hay contraseña, usar el número de empleado como contraseña por defecto
            empleado.set_password(validated_data.get('numero_empleado', ''))
        empleado.save()
        return empleado
